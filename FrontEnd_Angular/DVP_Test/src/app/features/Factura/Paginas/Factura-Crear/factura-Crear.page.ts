...
export class CrearFacturaPage {
  ...
  detalles = signal<FacturaDetalleLineaModel[]>([]);
  ...
  // Signal para manejar nombre e imagen de los productos en las líneas
  detallesConInfo = signal<
    (FacturaDetalleLineaModel & { nombre: string; imagen: string })[]
  >([]);
  ...
  agregarLinea(event: Event): void {
    ...
    this.facade.buscarProductosById(id).subscribe({
      next: (producto) => {
        ...
        this.detalles.update((det) => [...det, nuevaLinea]);
        this.detallesConInfo.update((det) => [...det, nuevaLinea]);
        this.recalcular();
      },
